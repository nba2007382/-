  ...
  <section class="section" >
      <div class="box">
        ...
        <el-button class="delete" type="danger" :icon="Delete" circle @click="deletshow=!deletshow"/>
          <div >
                <ul>
                    <li class="list move" v-for="item in list.data" :key="item.id" :jd_id='item.id'>
                        <el-button v-if='deletshow' @click="deletegoods($event)" class="delete" type="danger" :icon="CloseBold" circle />
                          <router-link :to="{path:'/jd',query:{id:item.id}}">
                              <div class="pic">                               
                                <img :src="item.img" :alt="item.title">                                
                              </div>
                                <p style="overflow:hidden;max-height:30px"> {{item.title}}</p>                            
                          </router-link>

                      
                        <label style="font-weight:bolder;color:red" >{{item.label}}</label>
...
setup(){
    ...
    const handleHref=()=>{
      axios.get('/api/monito/jd/addgoods',{
        params:{
          url:input
        }
      }).then(res=>{
    ...
    getList()
      const deletegoods=(event)=>{
          ...
          axios.delete('/api/monito/jd/delgoods',{
                data:{id}
          }).then(res=>{
                alert(res.data.message)
                getList()
    ...
    return{visible,input,handleHref,list,Delete,CloseBold,deletshow,deletegoods}
}
}
</script>
