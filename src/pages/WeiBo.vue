            ...
            <div class="content" :style="{boxShadow: `var(--el-box-shadow-lighter)`}" >
                  ...
                  >
                    <template v-slot="{ item, index, active }">
                      ...
                      >
                      <div  :key="item.id">
                                  <div  class="wbpro-scroller-item" style="margin-top:5px" :key="item.id" :id="item.id" :style="{boxShadow: `var(--el-box-shadow-lighter)`}">
                                    <p v-if="item.isTop" style="margin-top:5px;margin-left:5px;margin-bottom:5px;color:red">置顶</p>
                                    <div class="header" :key="item.user.id">
                                      <el-row>
                                        <img :src="item.user.profile_image_url" :alt="item.user.screen_name">
                                    ...
                                    <div class="wbpro-feed-content" :key="item.id">
                                      ...
                                      <div class="pic-list" :key="item.id">
                                        ...
                                        <div class="pic" v-for="(p,k) in item.pic_infos" :key="p.pic_id" @click="getPicList(k,item.pic_infos)">                          
                                            <video v-if="p.type=='livephoto'" :data-id="item.id" class="img_video" width="132" height="132" style="border-radius: 10px;" autoplay :poster="p.thumbnail.url" :src="p.video" type="video/mp4"  muted></video>
                                            <img v-else :data-id="item.id" :src="p.thumbnail.url"  style="width: 132px; height:132px;border-radius: 10px;" >                                             
                                        </div>
                                        </el-row>            
                                      </div>
                                    </div>
        ...
        <div v-if="previewIndex!==null" style="height:100%;width:70%;z-index:100;margin-left:50%;transform:translateX(-50%);background-color:#242529;">
           <el-carousel height="100vh" :initial-index="previewIndex" :autoplay="false">
              ...
              <el-row justify="center" align="middle" style="width:100%;height: 100%;">
                 <video v-if="p.type=='livephoto'" :src="p.video" type="livephoto" :poster="p.original.url"  autoplay controls muted></video>
...
export default {
    ...
    setup(){
      ...
      const getList=()=>{
        axios.get('/api/monito/wb/getwatchList').then(res=>{
          ...
          axios.get('/api/monito/wb/getlistContents',{
          params:{
  ...
  return function () {
    ...
    if (now - prev >= delay) {
      ...
      getList()
      return{watchList,contents,input,deletshow,visible,handleHref,setAvatorIndex,CloseBold,deleteAvator,conentTime,getPicList,previewIndex,previewList,scrollbarRef,ajaxContent}
    }
}
</script>
